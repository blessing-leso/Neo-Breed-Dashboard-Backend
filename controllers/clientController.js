...
export const assignClient = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json(
        `Client ${clientFullname} has been assigned to ${employeeFullname} `
      );
  } catch (error) {
    res.json({ error: error.message });
  }
...
export const unassignClient = async (req, res) => {
  ...
  try {
    ...
    employee.clients = employee.clients.filter(
      (clientObjId) => !clientObjId.equals(clientId)
    );
    ...
    res
      .status(200)
      .json(
        `Client ${clientFullname} has been unassigned from ${employeeFullname} `
      );
  } catch (error) {
    res.json({ error: error.message });
  }
